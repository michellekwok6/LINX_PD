...
#figuring out a good way to do a double integral with jax

f = lambda y, x: 1

print(dblquad(f, 0, np.pi/4, np.sin, np.cos))
...
# Define the integrand function (which is just 1 in this case)
def integrand(y, x):
    return 1
...
def outer_integral(x):
    # Inner integration with respect to y, from sin(x) to cos(x)
    result, error = quad(integrand, np.sin(x), np.cos(x), args=(x,))
    return result

# Compute the outer integral with respect to x
result, error = quad(outer_integral, 0, np.pi/4)


print(f"The result of the double integral is: {result}")
print(f"Estimated error: {error}")

def new_outer_integral(x):
    # Inner integration with respect to y, from sin(x) to cos(x)
    result, error = romb(1, np.sin(x), np.cos(x), args=(x,))
    return result

print(romb(new_outer_integral, 0, np.pi/4))